...
# LOW LEVEL FUNCTIONS ==========================================================
@njit(cache=True)
def _min_max_inverted_norm(results):
...
def _min_max_inverted_norm_parallel(run):
    ...
    for i in prange(len(q_ids)):
        normalized_run[i] = _min_max_inverted_norm(run[q_ids[i]])

    return convert_results_dict_list_to_run(q_ids, normalized_run)


# HIGH LEVEL FUNCTIONS =========================================================
def min_max_inverted_norm(run: Run) -> Run:
    """Apply `min_max_inverted norm` to a given run.
    ...
    normalized_run.run = _min_max_inverted_norm_parallel(run.run)
