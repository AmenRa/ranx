    ...
    for res in results:
        for doc_id in res.keys():
            if combined_results.get(doc_id, False) == False:
                ...
                combined_results[to_unicode(doc_id)] = np.median(scores)

    return combined_results


@njit(cache=True, parallel=True)
def _comb_med_parallel(runs):
    q_ids = TypedList(runs[0].keys())
    combined_results = create_empty_results_dict_list(len(q_ids))

    for i in prange(len(q_ids)):
        q_id = q_ids[i]
        combined_results[i] = _comb_med([run[q_id] for run in runs])

    return convert_results_dict_list_to_run(q_ids, combined_results)


# HIGH LEVEL FUNCTIONS =========================================================
def comb_med(runs: List[Run], name: str = "comb_med") -> Run:
    r"""Computes CombMED as proposed by [Fox et al.](https://trec.nist.gov/pubs/trec2/papers/txt/23.txt).
    ...
    Args:
        ...
        name (str): Name for the combined run. Defaults to "comb_med".
    ...
    run = Run()
    run.name = name
    run.run = _comb_med_parallel(TypedList([run.run for run in runs]))
    run.sort()
    return run
